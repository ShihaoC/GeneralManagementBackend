...
public class UserServiceImpl implements UserService {
    ...
    public Response<User> login(User user) {
        ...
        if (resultUser == null) {
            return new ResponseFactory<User>().getInstance(null, "用户名密码不正确", ResponseCode.ERROR);
        }
        String token = JwtUtil.getToken(resultUser.getUsername(), ROLE.valueOf(resultUser.getRole()));
        resultUser.setToken(token);
        return new ResponseFactory<User>().getInstance(resultUser, "success", ResponseCode.SUCCESS);
    }
    ...
    public Response<String> register(User user) {
        if (user == null) {
            return new ResponseFactory<String>().getInstance("表单内容为空", "error", ResponseCode.ERROR);
        }
        QueryWrapper<User> wrapper = new QueryWrapper<>();
        ...
        if(queryResult != null){
            return new ResponseFactory<String>().getInstance("已经存在","已经存在",ResponseCode.IS_EXISTS);
        }
        ...
        if (insert > 0) {
            return new ResponseFactory<String>().getInstance("注册成功", "success", ResponseCode.SUCCESS);
        }
        return new ResponseFactory<String>().getInstance("注册失败", Objects.requireNonNull(user).getUsername() == null ? "用户名不能为空" : "密码不能为空", ResponseCode.ERROR);
    }
}
